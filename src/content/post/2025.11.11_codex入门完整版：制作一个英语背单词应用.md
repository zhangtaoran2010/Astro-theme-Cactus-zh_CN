...
```
移除掉用户登陆后进入到 /protected 页面的重定向，不需要
```
...
请你根据数据结构，修改对应的UI展示。同时单词卡片的宽度调整为屏幕宽度。
````
...
请使用上面我给你的数据去渲染首页单词书。封面使用 `cover_url` 实现渲染。
````
...
输入上面的提示词去接入真实的一个数据库的数据。

![](/assets/images/image508.png)
...
请你替换 audio 参数和 type 参数。 
```

输入上面的提示词，来完成单词的发音功能的开发。如果说开发完成之后，进行一个实际的测试。

![](/assets/images/image510.png)

目前这个产品还是有很多可以优化的点。比如说，加入骨架图、加入单词的复习流程的算法，包括类似艾宾浩斯记忆一个记忆算法等。你可以自己去扩展这个产品，或者把它修改成小程序或iOS 应用，都是可以的。加入一些有趣的玩法。

**项目源码**

**1. 管理后台：[下载链接](https://pub-8d9c7b440bdc4316a94cd1a6ec45d0ce.r2.dev/codex-admin-nextjs-main.zip)**

**2. 前端H5：[下载链接](https://pub-8d9c7b440bdc4316a94cd1a6ec45d0ce.r2.dev/codex-h5-nextjs-main.zip)[](https://pub-8d9c7b440bdc4316a94cd1a6ec45d0ce.r2.dev/codex-admin-nextjs-main.zip)**
